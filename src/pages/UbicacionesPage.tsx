...
import type { Ubicacion } from '../lib/api';
import { useUbicaciones } from '../hooks/useData';
...
export default function UbicacionesPage({ onMenuOpen }: Props) {
  const { data, loading, reload } = useUbicaciones();
  const [page, setPage] = useState(1);
  const perPage = 15;
  const [editing, setEditing] = useState<Ubicacion | null>(null);
  const [formOpen, setFormOpen] = useState(false);
  const [nombre, setNombre] = useState('');
  const [deleteItem, setDeleteItem] = useState<Ubicacion | null>(null);
  const [saving, setSaving] = useState(false);

  const openNew = () => { setEditing(null); setNombre(''); setFormOpen(true); };
  const openEdit = (row: Ubicacion) => { setEditing(row); setNombre(row.nombre); setFormOpen(true); };

  const handleSave = async () => {
    if (!nombre.trim()) return;
    setSaving(true);
    if (editing) await apiFetch(`/ubicacion/${editing.id}`, { method: 'PUT', body: JSON.stringify({ nombre }) });
    else await apiFetch('/ubicacion', { method: 'POST', body: JSON.stringify({ nombre }) });
    setSaving(false);
    setFormOpen(false);
    reload();
  };
  ...
  const handleDelete = async () => {
    ...
    await apiFetch(`/ubicacion/${deleteItem.id}`, { method: 'DELETE' });
    setDeleteItem(null);
    reload();
  };
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      />
      <Modal open={formOpen} onClose={() => setFormOpen(false)} title={editing ? 'Editar Ubicación' : 'Nueva Ubicación'} size="sm">
        <div className="space-y-4">
          <input type="text" value={nombre} onChange={e => setNombre(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleSave(); }}
            className="w-full bg-slate-700 border border-slate-600 text-slate-100 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500" placeholder="Ej: Santiago" autoFocus />
