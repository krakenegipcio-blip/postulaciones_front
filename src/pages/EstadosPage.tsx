...
import { useEstados } from '../hooks/useData';
...
export default function EstadosPage({ onMenuOpen }: Props) {
  const { data, loading, reload } = useEstados();
  ...
  const [deleteItem, setDeleteItem] = useState<Estado | null>(null);
  const [saving, setSaving] = useState(false);

  const openNew = () => { setEditing(null); setNombre(''); setColor('#6b7280'); setFormOpen(true); };
  const openEdit = (row: Estado) => { setEditing(row); setNombre(row.nombre); setColor(row.color_hex); setFormOpen(true); };

  const handleSave = async () => {
    if (!nombre.trim()) return;
    setSaving(true);
    if (editing) {
      await apiFetch(`/estado/${editing.id}`, { method: 'PUT', body: JSON.stringify({ nombre, color_hex: color }) });
    } else {
      await apiFetch('/estado', { method: 'POST', body: JSON.stringify({ nombre, color_hex: color }) });
    }
    setSaving(false);
    setFormOpen(false);
    reload();
  ...
  const handleDelete = async () => {
    ...
    await apiFetch(`/estado/${deleteItem.id}`, { method: 'DELETE' });
    setDeleteItem(null);
    reload();
  };
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <MaintainerTable
        title="Mantenedor de Estados"
        icon={<Tag size={18} className="text-slate-400" />}
        rows={paged}
        columns={[
          { key: 'nombre', label: 'Nombre', render: (r: Estado) => <Badge label={r.nombre} color={r.color_hex} size="md" /> },
          { key: 'color_hex', label: 'Color Hex', render: (r: Estado) => (
            <div className="flex items-center gap-2">
              <span className="w-4 h-4 rounded-full border border-slate-600" style={{ backgroundColor: r.color_hex }} />
      ...
      <Modal open={formOpen} onClose={() => setFormOpen(false)} title={editing ? 'Editar Estado' : 'Nuevo Estado'} size="sm">
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Badge label={nombre || 'Estado'} color={color} size="md" />
          </div>
